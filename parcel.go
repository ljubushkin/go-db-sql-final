...
func (s ParcelStore) Get(number int) (Parcel, error) {
	row := s.db.QueryRow("SELECT id, client, status, address, createdAt FROM parcel WHERE number = :number", sql.Named("number", number))
...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	rows, err := s.db.Query("SELECT id, client, status, address, createdAt FROM parcel WHERE client = :client", sql.Named("client", client))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
